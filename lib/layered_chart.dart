...
import 'package:flutter_covid_plot/catmull.dart';
...
class LayeredChart extends StatefulWidget {
  ...
  final List<MonthLabel> milestones;
  ...
  void buildPaths(
    ...
    for (int i = 0; i < m; i++) {
      ...
      for (int j = 0; j < numPoints; j++) {
        ...
        curvePoints[j] =
            MathUtils.map(max(0, cpv.value), 0, maxValue, 0, graphHeight);
      }
      paths[i] = Path();
      capPaths[i] = Path();
...
class ChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    int m = dataToPlot.length;
    int days = dataToPlot[0].series.length;
    ...
    {
      for (int i = 0; i < milestones.length; i++) {
        MonthLabel milestone = milestones[i];
        double p = (milestone.weekNum.toDouble() / days) + (1 - amount);
        if (p < 1) {
    ...
    for (int i = m - 1; i >= 0; i--) {
...

      {
        ...
        canvas.translate(textX, textY);
        int index = (amount * days).toInt();
        if (index >= days) {
          index = days - 1;
        }
        int caseNumber = dataToPlot[i].series[index];
        TextSpan span = TextSpan(
          style: TextStyle(
              color: Color.fromARGB(255, 255, 255, 255), fontSize: 16),
          text: "${dataToPlot[i].label.toUpperCase()}: $caseNumber cases",
        );
        TextPainter tp = TextPainter(
            text: span,
            textAlign: TextAlign.left,
